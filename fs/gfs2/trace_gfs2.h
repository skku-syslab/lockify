...
/* Trace unlock time in gdlm_put_lock */
TRACE_EVENT(gfs2_glock_unlock_time,

	TP_PROTO(const struct gfs2_glock *gl, s64 unlock_time),

	TP_ARGS(gl, unlock_time),

	TP_STRUCT__entry(
		__field(        dev_t,  dev                     )
		__field(	u64,	glnum			)
		__field(	u32,	gltype			)
		__field(	s64,	unlock_time		)
	),
	...
	TP_fast_assign(
		...
		__entry->gltype		= gl->gl_name.ln_type;
		__entry->glnum		= gl->gl_name.ln_number;
		__entry->unlock_time	= unlock_time;
	),

	TP_printk("%u,%u glock %d:%lld unlock_time:%lld ns",
		  MAJOR(__entry->dev), MINOR(__entry->dev),
		  __entry->gltype, (unsigned long long)__entry->glnum,
		  __entry->unlock_time)
);
