		       ...
		       int start, int end)
{
	...
	// @pty
	lkb->lkb_wait_timestamp = 0;
	lkb->lkb_parent_lkid = 0;
	INIT_LIST_HEAD(&lkb->lkb_parent_link);
	atomic_set(&lkb->lkb_total_children, 0);
	spin_lock_init(&lkb->lkb_child_lock);
	INIT_LIST_HEAD(&lkb->lkb_child_waiters);
	init_waitqueue_head(&lkb->lkb_wait_queue);
		       ...
		       struct dlm_args *args)
{
	...
	// @pty
	error = wait_event_interruptible(lkb->lkb_wait_queue, atomic_read(&lkb->lkb_total_children) == 0);
	if (error)
		return error;
	     ...
	     void (*bast) (void *astarg, int mode))
{
	...
	// @pty
	if (flags & DLM_LKF_NOTIFY) {
		lkb->lkb_parent_lkid = parent_lkid;
	}
...
static int send_notify(struct dlm_rsb *r, struct dlm_lkb *lkb)
{
	struct dlm_message *ms;
	struct dlm_mhandle *mh;
	struct dlm_lkb *parent_lkb;
	int to_nodeid, error;

	if (lkb->lkb_parent_lkid)
	{
		error = find_lkb(r->res_ls, lkb->lkb_parent_lkid, &parent_lkb);
		if (error)
			return error;

		spin_lock(&parent_lkb->lkb_child_lock);
		list_add_tail(&lkb->lkb_parent_link, &parent_lkb->lkb_child_waiters);
		atomic_inc(&parent_lkb->lkb_total_children);
		spin_unlock(&parent_lkb->lkb_child_lock);

		dlm_put_lkb(parent_lkb);
	}
				 ...
				 const struct dlm_message *ms)
{
	...
	if (lkb->lkb_parent_lkid) {
		error = find_lkb(ls, lkb->lkb_parent_lkid, &parent_lkb);
		if (error) {
			log_error(ls, "%s no parent lkid %x", __func__,
					lkb->lkb_parent_lkid);
			return;
		}

		spin_lock(&parent_lkb->lkb_child_lock);
		list_del(&lkb->lkb_parent_link);
		if(atomic_dec_and_test(&parent_lkb->lkb_total_children))
			wake_up_interruptible(&parent_lkb->lkb_wait_queue);
		spin_unlock(&parent_lkb->lkb_child_lock);
		dlm_put_lkb(parent_lkb);
	}
