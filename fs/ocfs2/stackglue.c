...
// @pty
int ocfs2_dlm_lock_notify(struct ocfs2_cluster_connection *conn,
		   int mode,
		   struct ocfs2_dlm_lksb *lksb,
		   u32 flags,
		   void *name,
		   ...
		   uint32_t parent_lkid)
{
	if (!lksb->lksb_conn)
		...
		BUG_ON(lksb->lksb_conn != conn);
	return active_stack->sp_ops->dlm_lock_notify(conn, mode, lksb, flags,
					      name, namelen, parent_lkid);
}
EXPORT_SYMBOL_GPL(ocfs2_dlm_lock_notify);
