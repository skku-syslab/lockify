...
struct ocfs2_parent_lock_ctx {
	struct list_head	plc_notify_locks;
	spinlock_t		plc_lock;
	int			plc_pending_count;
	struct completion	plc_all_confirmed;
	struct ocfs2_lock_res	*plc_parent_lockres;
};
				 ...
				 uint32_t parent_lkid)
{
	...
	{
		ret = ocfs2_dlm_lock_notify(osb->cconn,
									level,
									&lockres->l_lksb,
...
// @pty
static inline int ocfs2_cluster_lock_notify(struct ocfs2_super *osb,
				     struct ocfs2_lock_res *lockres,
				     int level,
				     u32 lkm_flags,
				     int arg_flags,
				 ...
				 uint32_t parent_lkid)
{
	...
	return ocfs2_lock_create(osb, lockres, level, lkm_flags, parent_lkid);
}
...
// @pty
int ocfs2_create_new_inode_locks(struct inode *inode, uint32_t parent_lkid)
{
	int ret;
	...
	ret = ocfs2_create_new_lock(osb, &OCFS2_I(inode)->ip_rw_lockres, 1, 1, parent_lkid);
	if (ret) {
		mlog_errno(ret);
		goto bail;
	}
	 ...
	 */
	ret = ocfs2_create_new_lock(osb, &OCFS2_I(inode)->ip_inode_lockres, 1, 0, parent_lkid);
	if (ret) {
		mlog_errno(ret);
		goto bail;
	}

	ret = ocfs2_create_new_lock(osb, &OCFS2_I(inode)->ip_open_lockres, 0, 0, parent_lkid);
	if (ret)
		mlog_errno(ret);
...
int ocfs2_file_lock(struct file *file, int ex, int trylock)
{
	...
	if (!(lockres->l_flags & OCFS2_LOCK_ATTACHED)) {
		 ...
		 */
		ret = ocfs2_lock_create(osb, lockres, DLM_LOCK_NL, 0, 0);
		if (ret < 0) {
			mlog_errno(ret);
			goto out;
		}
...
// @pty
int ocfs2_dentry_lock_notify(struct dentry *dentry, int ex, uint32_t parent_lkid)
{
	int ret;
	int level = ex ? DLM_LOCK_EX : DLM_LOCK_PR;
	struct ocfs2_dentry_lock *dl = dentry->d_fsdata;
	...
	ret = ocfs2_cluster_lock_notify(osb, &dl->dl_lockres, level, DLM_LKF_NOTIFY, 0, parent_lkid);
	if (ret < 0)
		mlog_errno(ret);
