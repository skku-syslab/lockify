...
static int ocfs2_mknod_locked_notify(struct ocfs2_super *osb,
			      struct inode *dir,
			      struct inode *inode,
			      dev_t dev,
			      struct buffer_head **new_fe_bh,
		       ...
		       dev_t dev)
{
	...
	// @pty
	parent_lkid = oi->ip_inode_lockres.l_lksb.lksb_fsdlm.sb_lkid;
	...
	// @pty
	status = ocfs2_mknod_locked_notify(osb, dir, inode, dev,
				    &new_fe_bh, parent_fe_bh, handle,
				    inode_ac, parent_lkid);
	if (status < 0) {
		mlog_errno(status);
		goto leave;
	}
	...
	status = ocfs2_dentry_attach_lock_notify(dentry, inode,
					  OCFS2_I(dir)->ip_blkno, parent_lkid);
	if (status) {
		mlog_errno(status);
		goto roll_back;
	}
				...
				u64 fe_blkno, u64 suballoc_loc, u16 suballoc_bit)
{
	...
	*new_fe_bh = sb_getblk(osb->sb, fe_blkno);
	...
	if (!ocfs2_mount_local(osb)) {
		status = ocfs2_create_new_inode_locks(inode, 0);
		if (status < 0)
			mlog_errno(status);
	}
...
// @pty
static int ocfs2_mknod_locked_notify(struct ocfs2_super *osb_unused,
			      struct inode *dir,
			      struct inode *inode,
			      dev_t dev,
			      struct buffer_head **new_fe_bh,
				  ...
				  uint32_t parent_lkid)
{
	...
	*new_fe_bh = sb_getblk(osb->sb, fe_blkno);
	...
	ocfs2_ci_set_new(osb, INODE_CACHE(inode));
	if (!ocfs2_mount_local(osb)) {
		status = ocfs2_create_new_inode_locks(inode, parent_lkid);
		if (status < 0)
			mlog_errno(status);
	}
